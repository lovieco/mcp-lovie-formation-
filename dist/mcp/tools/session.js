...
exports.formationResumeTool = exports.formationGetStatusTool = exports.formationDescribeBusinessTool = exports.formationStartTool = void 0;
...
const handleFormationStart = async (args, store) => {
    // Get userId from authenticated session
    const userId = (0, auth_1.getUserId)();
...
exports.formationDescribeBusinessTool = {
...
const handleFormationDescribeBusiness = async (args, store) => {
    const sessionId = args.sessionId;
    const businessDescription = args.businessDescription;
    if (!businessDescription || businessDescription.length < 10) {
        throw (0, errors_1.requiredFieldError)('businessDescription');
    }
    ...
    session.currentStep = types_1.FormationStep.BUSINESS_DESCRIBED;
...
exports.formationGetStatusTool = {
...
const handleFormationGetStatus = async (args, store) => {
    const sessionId = args.sessionId;
    const session = await (0, session_1.loadSession)(sessionId, store);
    const companyType = session.companyDetails?.companyType;
    const completedSteps = (0, session_1.getCompletedSteps)(session.currentStep, companyType);
    const remainingSteps = (0, session_1.getRemainingSteps)(session.currentStep, companyType);
    const percentComplete = (0, session_1.calculateProgress)(session.currentStep, companyType);
    return {
        sessionId: session.sessionId,
        userId: session.userId,
...
const handleFormationResume = async (args, store) => {
    const sessionId = args.sessionId;
    const session = await (0, session_1.loadSession)(sessionId, store);
    const companyType = session.companyDetails?.companyType;
    const completedSteps = (0, session_1.getCompletedSteps)(session.currentStep, companyType);
    ...
    return {
        ...
        progress: {
            completedSteps,
            remainingSteps,
            percentComplete: (0, session_1.calculateProgress)(session.currentStep, companyType),
        },
        nextAction,
...
function registerSessionTools() {
    (0, index_1.registerTool)(exports.formationStartTool, handleFormationStart);
    (0, index_1.registerTool)(exports.formationDescribeBusinessTool, handleFormationDescribeBusiness);
    (0, index_1.registerTool)(exports.formationGetStatusTool, handleFormationGetStatus);
    (0, index_1.registerTool)(exports.formationResumeTool, handleFormationResume);
}
